...
@testset "Test `parse*` functions" begin
    ...
    @test parse(Float64, f"1.2345678d-8") ≈ 1.2345678e-8
    @test parse(Float64, f"1.2345677d-8") ≈ 1.2345677e-8
    @test parse(Float64, f"1.2345676d-8") ≈ 1.2345676e-8
    @test !(parse(Float64, f"1.2345676d-8") ≈ 1.2345677e-8)
    @test !(parse(Float64, f"1.2345676d-8") ≈ 1.2345678e-8)
    ...
    @test parse(String, f"\"./1\"") === "./1"
    @test parse(String, f"\"./\"") === "./"
    @test parse(String, f"\"./1.1\"") === "./1.1"
    @test parse(String, f"\"./s=1.1\"") === "./s=1.1"
    @test parse(String, f"\"./a=[1,2,3,4]\"") === "./a=[1,2,3,4]"
